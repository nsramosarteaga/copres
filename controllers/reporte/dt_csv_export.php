    ...
    isset($_POST['csv_data']) && !empty($_POST['csv_data']))
{
    ...
    // Write CSV To Disk
    $csv_data = explode("\r\n", $_POST['csv_data']);
    $archivo = uniqid($_POST['csv_type'] .'_', true) .'.csv';
    $csv_file_name = $config['path_aplicacion'].'files/csv/'.$archivo;
    $csv_file_url = $config['url_aplicacion'].'files/csv/'.$archivo;    
    ...
    echo file_exists($csv_file_name) ? $csv_file_url : '';
}
